...
app.get(/^\/alert(\d+)$/, (req, res) => {
  ...
  res.setHeader("Cache-Control", "no-store");
  res.type("html").send(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#091018"><meta name="apple-mobile-web-app-capable" content="yes"><title>Howard's Orders</title><link rel="manifest"><link rel="icon" href="/alert-icon.svg"><link rel="stylesheet" href="/alert-app.css"></head><body><header><h1>HOWARD'S ORDERS #${req.params.shop}</h1><div>STATUS: <span id="status" class="status reconnecting">RECONNECTING</span></div><nav id="orderTabs" aria-label="Order status"><button data-tab="active" class="selected">Active</button><button data-tab="complete">Complete</button><button id="refreshOrders" type="button">Refresh</button></nav></header><main><section id="listView"><div id="alerts"></div></section><section id="detailView" class="hidden details"><button id="detailAction"></button><div id="detailBody"></div></section></main><script src="/order-alarm.js"></script><script src="/alert-app.js" defer></script></body></html>`);
});
