...
(async () => {
  try {
    ...
    const push=alertSystem.buildPushoverValues(first,"token","user");assert.strictEqual(push.priority,"2");assert.strictEqual(push.retry,"60");assert.strictEqual(push.expire,"1800");assert.strictEqual(push.title,"Pat - UberEats - Delivery");assert.ok(Object.hasOwn(push,"title"));assert.strictEqual(push.message,"\u200B");assert.strictEqual(push.device,"front-counter");assert.notStrictEqual(push.device,process.env.PUSHOVER_DEVICE_2);assert.doesNotMatch(push.message,/Burger|Pickle|Shake|Cherry|A1|901|312|Schedule|Ordered/i);assert.strictEqual(Object.hasOwn(push,"url"),false);assert.strictEqual(Object.hasOwn(push,"url_title"),false);
    const example=alertSystem.createShopAlert({shop:1,jobReference:"title-example",source:"DoorDash",type:"Delivery",customer:"Tristan G."});assert.strictEqual(alertSystem.buildPushoverValues(example,"token","user").title,"Tristan G. - DoorDash - Delivery");
    const push2=alertSystem.buildPushoverValues(second,"token","user");assert.strictEqual(push2.title,"Customer - DoorDash - Pickup");assert.strictEqual(push2.message,"\u200B");assert.strictEqual(push2.device,"kitchen-tablet");assert.notStrictEqual(push2.device,process.env.PUSHOVER_DEVICE_1);
    ...
    const pushoverSource=fs.readFileSync(path.join(__dirname,"../alert-system.cjs"),"utf8");assert.doesNotMatch(pushoverSource,/New Order - Shop|url_title|View Order/);
