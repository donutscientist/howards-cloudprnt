...
(async () => {
  try {
    ...
    const workerListeners={};let shownNotification;
    vm.runInNewContext(worker,{URL,caches:{},fetch(){},self:{location:{origin:"https://alerts.example"},clients:{},registration:{showNotification(title,options){shownNotification={title,options};return Promise.resolve();}},addEventListener(type,listener){workerListeners[type]=listener;}}});
    workerListeners.push({data:{json:()=>({title:"New Order - Shop #1",body:"Pat\nUberEats - Delivery",shop:1,order:"alert-1"})},waitUntil(promise){return promise;}});
    assert.strictEqual(shownNotification.title,"New Order - Shop #1");assert.strictEqual(shownNotification.options.body,"Pat\nUberEats - Delivery");assert.deepStrictEqual(Array.from(shownNotification.options.vibrate),[300,150,300,150,500]);assert.notStrictEqual(shownNotification.options.silent,true);assert.strictEqual(shownNotification.options.data.shop,1);assert.strictEqual(shownNotification.options.data.order,"alert-1");
