...
(async () => {
  try {
    ...
    let plays=0, intervals=0, clears=0, callback;
    const fakeTimers={setInterval(fn,ms){intervals++;callback=fn;assert.strictEqual(ms,ORDER_ALARM_REPEAT_MS);return 7},clearInterval(id){clears++;assert.strictEqual(id,7)}};
    const controller=createOrderAlarmController(()=>plays++,fakeTimers);
    const newOne={status:"active",acknowledged:false},newTwo={status:"active",acknowledged:false};
    controller.update([newOne]);assert.strictEqual(controller.isRunning(),true);assert.strictEqual(plays,1);assert.strictEqual(intervals,1);callback();assert.strictEqual(plays,2);
    controller.update([newOne,newTwo]);assert.strictEqual(intervals,1);assert.strictEqual(plays,2);
