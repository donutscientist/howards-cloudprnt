...
(async () => {
  try {
    ...
    const push=alertSystem.buildPushoverValues(first,"token","user");assert.strictEqual(push.priority,"2");assert.strictEqual(push.retry,"60");assert.strictEqual(push.expire,"1800");assert.strictEqual(push.title,"Pat - UberEats - Delivery");assert.strictEqual(push.message,"Pat\nUberEats - Delivery\n\nOrder #A1\nSchedule: ASAP\nOrdered on: 07/28/2026 6:18 AM\n\nCustomer Phone: +1 901-555-1234\nCourier Phone: +1 312-555-5678\n\nTotal: 3 Items\n\n2x Burger\n    3x Pickle\n\n1x Shake\n    2x Cherry");assert.strictEqual(push.device,"front-counter");assert.notStrictEqual(push.device,process.env.PUSHOVER_DEVICE_2);assert.doesNotMatch(push.message,/\$|payment|tender|card|subtotal|tax|tip|token|secret|•/i);assert.match(push.url,new RegExp(`/alert1\\?key=alert-test-key&order=${first.id}$`));assert.strictEqual(push.url_title,"View Details");assert.doesNotMatch(push.url,new RegExp(`${token1}|A1`));
    const example=alertSystem.createShopAlert({shop:1,jobReference:"title-example",source:"DoorDash",type:"Delivery",customer:"Tristan G."});const examplePush=alertSystem.buildPushoverValues(example,"token","user");assert.strictEqual(examplePush.title,"Tristan G. - DoorDash - Delivery");assert.strictEqual(examplePush.message,"Tristan G.\nDoorDash - Delivery\n\nTotal: 0 Items");
    const push2=alertSystem.buildPushoverValues(second,"token","user");assert.strictEqual(push2.title,"Customer - DoorDash - Pickup");assert.strictEqual(push2.message,"Customer\nDoorDash - Pickup\n\nTotal: 1 Items\n\n1x Donut");assert.match(push2.url,new RegExp(`/alert2\\?key=alert-test-key&order=${second.id}$`));assert.strictEqual(push2.device,"kitchen-tablet");assert.notStrictEqual(push2.device,process.env.PUSHOVER_DEVICE_1);
