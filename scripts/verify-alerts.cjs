...
(async () => {
  try {
    ...
    const push=alertSystem.buildPushoverValues(first,"token","user");assert.strictEqual(push.priority,"2");assert.strictEqual(push.retry,"60");assert.strictEqual(push.expire,"1800");assert.strictEqual(push.title,"Pat");assert.strictEqual(push.message,"UberEats\nDelivery");assert.match(push.url,new RegExp(`/alert1\\?key=alert-test-key&order=${first.id}$`));assert.strictEqual(push.url_title,"View Order");assert.doesNotMatch(push.message,/Pat|phone|address|\$/i);
